...
|  | **Free版** `python start.py` | **Pro版** `python start_pro.py` |
|---|---|---|
| **模型选择** | 单一模型（知识库推荐） | 多模型竞赛 — 每问2-3种方法对比，AI选最优 |
| **灵敏度** | 基本OAT | Tornado图 + Sobol指数 + 蒙特卡洛 |
| **误差分析** | MAPE一个数 | 残差图 + QQ图 + 预测区间 + DW检验 + 排名稳定性 |
| **图表** | 单张散落 | 2×2/1×3专业多面板图表组 |
| **论文写作** | 模板填充 | AI拿到真实数值写分析（现象→原因→建议） |
| **论文页数** | 12-15页 | 20-25页 |
| **获奖潜力** | 省二~省三 | 省一 |
| **预估耗时** | ~2 分钟 | ~10-20 分钟 |
| **API费用** | ¥0.5-1 | ¥3-5（20-25次DeepSeek调用） |
| **费用** | 永远免费 | 永远免费 |

> 💰 Pro版多花 ¥2-4，多等10分钟，论文质量从省二提升到省一。比请人代写便宜1000倍。
